...
import cl.hyh.redpagos.caja.trx.CodigoAutorizacionOffline;

/**
 * @author Felipe Hernandez - Hernandez e Hidalgo Ltda.
 *
 */
public class ChequeBaseOffline extends MedioPago{
    protected int estado = 0;
    public CodigoAutorizacionOffline claveSup = null;
    boolean isRut = true;
    ...
    protected int etapaIngresoCheque(ICajaView vista){
        ...
        if(Base.pos.micr.isLeido()){
            ...
            if( this instanceof ChequeOffline ){
...
//			Tools.logStackTrace(Base.logger, e1);
            ...
            JOptionPane.showMessageDialog(null, "En modo Contingencia no se realiza Verificación con Orsan.", "Continuar", JOptionPane.INFORMATION_MESSAGE);
            estado = 100;
            // Aca se invoca directamente al metodo que solicita Cod de autorizacion
            claveSup = new CodigoAutorizacionOffline();
...
//            if( vista.showMyConfirmDialog("Confirme por favor","Cheque rechazado: " + respCheque.getRetDesc() + "\nSelecione SI para Rechazar medio de pago o NO para solicitar autorizacion del supervisor")
