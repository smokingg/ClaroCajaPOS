 ...
 *
 */
public class TrxClaroRut implements ITrxBase{
	int estado = 0;
	String codigo = "";
	Datos data;
	DocumentoPago doc;
	...
	public int execute(ICajaView vista, int key, Datos datos){ 
		...
		case 1:
			...
			/** este campo no existe en DTO Caja
			...
			/**
        		in.setHeaderIn(hIn);
            	in.setRut(rut);
            	in.setTipoConsulta("ConsultaDetalle");
			 */
		...
		case 2:
			...
			if("VTV".equalsIgnoreCase(trv) || "ONE".equalsIgnoreCase(trv)){
				vista.paintButtons(Tools.getBotones(1));
			}else{
			...
			return ICajaView._WAITFORACTION;
				...
				JOptionPane.showMessageDialog(null, "NO EXISTEN DOCUMENTOS PARA EL RUT: " + vista.getEntryText(), "Continuar", JOptionPane.INFORMATION_MESSAGE);
				return 20;
			}
		...
		case 4:
			...
			try {
				...
				AprobacionDescuentoResponseDTO response = pr.obtenerAprobacionDescuento(request);
		...
		case 10:
			...
			clientesCuentas = new String[opOut.getDetalleDocumentos().length + 1];
			vista.hideAllEntries();
			vista.setEntryTitle( "Consulta por Rut - Detalle Documentos", true );
			...
			for(int i = 0; i < opOut.getDetalleDocumentos().length; i++){
				...
				DetalleDocumento auxIt = opOut.getDetalleDocumentos(index);
				String fecha = auxIt.getFechaEmisionDocumento();
				clientesCuentas[i + 1] = String.format(" %13s %5s %13s %13s $%12s", auxIt.getNumeroCuenta() , auxIt.getTipoDocumento(), "No trae !!", auxIt.getFechaVencimientoDocumento(), Format.formatMontoPantalla((long)auxIt.getSaldoNeto()));
			}
		...
		case 130:
			...
			for(int i = 0 ; i < out.getDocumentos().length ; i++){
				if(documento.getCuentaUnica().equals(out.getDocumentos()[i].getCuentaUnica()) && !out.getDocumentos()[i].getCuentaUnica().equals("0")){
					DocumentoVTR auxVtr = out.getDocumentos()[i];
					try {
						doc = FactoryDocumentoPago.makeInstance("DocumentoVtr");
					} catch (BaseException e) {
						Tools.logStackTrace(Base.logger, e);
					...
					((DocumentoVtr)doc).llenarDatos(auxVtr);
					doc.getDatos().setValue("Rut", out.getCliente().getRutCliente());
					doc.getDatos().setValue("RutCliente", Tools.limpiarRut(out.getCliente().getRutCliente()));
					doc.getDatos().setValue("Cliente", out.getCliente().getNombreCliente());
					if(!doc.isIngresable(vista.getOperTRV().getCarroCompras())){
		...
		case 201:            	
			...
			if (c2.before(c1) || c2.equals(c1)) {
				...
				try {
					...
					vista.hideBusyWindow();
					//JOptionPane.showMessageDialog(null, "Error de conexión", "Continuar", JOptionPane.INFORMATION_MESSAGE);
					JOptionPane.showMessageDialog(null, "Error de conexión : " + e2.getMessage(), "Continuar", JOptionPane.INFORMATION_MESSAGE);
					Tools.logStackTrace(Base.logger, e2);
					return 20;
				}
		...
		case 22:
			vista.paintButtons(Tools.getBotones(-1));
			vista.hideAllEntries();
			vista.setEntryTitle( "Consulta por Rut - Detalle Item", true );
			vista.setEntryTextArea("Desc. item: " + doc.getDatos().getStringValue("DescripcionItem")
					...
					+"\nNumero Cuenta: " + doc.getDatos().getStringValue("NumeroCuenta")
					+"\nNumero Documento: " + cuentaDocClaro.getFolioDocumento()
					+"\nSaldo: " + Format.formatMonto(doc.getDatos().getLongValue("Monto"))
					+"\nOrigen" + doc.getDatos().getStringValue("SistemaOrigen")
					, true);
			estado = 23;
			return ICajaView._WAITFORACTION;
		...
		case 23:
			if(!doc.isIngresable(vista.getOperTRV().getCarroCompras())){
				//Base.logger.info("Documento ya existe en carro de compras");
				//JOptionPane.showMessageDialog(null, "Documento ya existe en carro de compras", "Continuar", JOptionPane.INFORMATION_MESSAGE);
				Base.logger.info("Item ya existe o pertenece a un Documento en el carro de compras");
				JOptionPane.showMessageDialog(null, "Item ya existe o pertenece a un Documento en el carro de compras", "Continuar", JOptionPane.INFORMATION_MESSAGE);
				//estado = 241;
				estado = 202;
				return ICajaView._NOWAITFORACTION;
			}
			...
			if(Base.getEdicion()){
				if(doc.isEditable(vista.getOperTRV().getCarroMediosPago(), vista.getOperTRV().getCarroCompras(), doc.getDatos().getLongValue("Monto"))){
					Base.logger.info("El Item se puede agregar al carro, el monto es menor al de los Medios de pago");
				}else{
					Base.logger.info("El Item No se puede agregar al carro, el monto es mayor al de los Medios de Pago");
					JOptionPane.showMessageDialog(null, "El Item No se puede agregar al carro, el monto es mayor al de los Medios de Pago", "Continuar", JOptionPane.INFORMATION_MESSAGE);
					estado = 202;
					return ICajaView._NOWAITFORACTION;
				}
			}
