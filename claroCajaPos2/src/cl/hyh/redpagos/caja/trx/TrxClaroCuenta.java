...
/**
 * Implementacion de la transaccion de consulta por Cuenta Claro
 * @author cbriones
 *
 */
public class TrxClaroCuenta implements ITrxBase{
	int estado = 0;
	String codigo = "";
	Datos data;
	DocumentoPago doc;
	...
	//DetalleCuenta cuentaDocClaro = null;
	...
	public int execute(ICajaView vista, int key, Datos datos){ 
		...
		case 1:	            
			...
			/** Validar esta implementacion ???
	            if(codigo.length() == 25){
	            	codigo = codigo.substring(0,9);	
	            }
	            else if(codigo.length() == 28){
	            	codigo = codigo.substring(2,11);	
	            }
			 */

			consultaCuenta = codigo;
			...
			opIn.setCaja(caja);
			opIn.setTipoOperacion("cta"); // se setea el tipo de busqueda que debe realizar en este caso es Cuenta !!
			Base.logger.info("Valor Tipo Operacion: "+  opIn.getTipoOperacion());
			opIn.setValorOperacion(consultaCuenta);
		...
		case 2:
			...
			// TODO validar si debe rescatar Documentos !!!
			...
			// TODO validar implementacion del run como atributo ???
		...
		case 12:
			...
			vista.hideAllEntries();
			vista.setEntryTitle( "Consulta por Cuenta - Detalle Documentos", true );
			vista.setEntryTextArea("Número Documento: " + doc.getDatos().getStringValue("NumeroDocumento")
		...
		case 22:
			vista.paintButtons(Tools.getBotones(-1));
			vista.hideAllEntries();
			vista.setEntryTitle( "Consulta por Cuenta - Detalle Servicio", true );
		...
		case 241:
			...
			vista.hideAllEntries();
			int posArray = 0;
			vista.setEntryTitle( "Consulta por Cuenta - Detalle Servicios", true );
