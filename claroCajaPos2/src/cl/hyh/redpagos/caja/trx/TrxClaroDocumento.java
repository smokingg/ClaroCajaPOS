...
public class TrxClaroDocumento implements ITrxBase{
	...
	public int execute(ICajaView vista, int key, Datos htParam) {
		...
		case 1:	            
			...
			/** Comentado, validar si debe restringirse de esta forma el formato ??
	            if(codigo.length() == 25){
			...
			opIn.setCaja(caja);
			opIn.setTipoOperacion("doc"); // se setea el tipo de busqueda que debe realizar en este caso es Documento !!
		...
		case 2:
			...
			vista.setEntryTitle( "Consulta por Documento", true );
		...
		case 10:
			...
			clientesCuentas = new String[opOut.getDetalleDocumentos().length + 1];
			vista.hideAllEntries();
			vista.setEntryTitle( "Consulta por Documento - Detalle Documentos", true );
			...
			for(int i = 0; i < opOut.getDetalleDocumentos().length; i++){ 	
				...
				//String fecha = Tools.getFecha(aux.get);
