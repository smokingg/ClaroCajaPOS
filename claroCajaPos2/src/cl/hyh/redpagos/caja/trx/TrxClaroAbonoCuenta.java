 ...
 *
 */
public class TrxClaroAbonoCuenta extends TrxClaroAbono{
    ...
    public int execute(ICajaView vista, int key, Datos datos){ 
        ...
        switch( estado ) {
            case 0:
                estado = 1;
                vista.setEntryMessage( "Ingrese Cuenta (Formato: 99999999)", true );
            ...
            case 1:
                ...
                if("".trim().equalsIgnoreCase(codigo)){
                    estado = 1;
                    vista.setEntryMessage( "No ha ingresado ninguna Cuenta - Ingrese nuevamente", true );
                    vista.setEntryTextLabel("Ingrese Cuenta:", true);
                    vista.setEntryText("", true, false, false,null,null);
                    return ICajaView._WAITFORACTION;
                }
                ...
                if(!Tools.validaNumerico(codigo)){
                    estado = 1;
                    vista.setEntryMessage( "Cuenta no válida - Ingrese nuevamente", true );
                    vista.setEntryTextLabel("Ingrese Cuenta:", true);
                    vista.setEntryText("", true, false, false,null,null);
                    return ICajaView._WAITFORACTION;
                }
                ...
                opIn.setCaja(caja);
                opIn.setTipoOperacion("ABONO"); // se setea el tipo de busqueda que debe realizar en este caso es CUENTA !!
				...
				//PREGUNTO SI EL CODIGO DE RETORNO ES DISTINTO DE 0 O EL DETALLE SEA NULO
				//O EL NUMERO DE CUENTA VIENE VACIO ("")
				if(r.getRetCode() != 0 || opOut.getDetalleCuentas() == null || 
						opOut.getDetalleCuentas()[0].getNumeroCuenta() == null || 
						opOut.getDetalleCuentas()[0].getNumeroCuenta().equals("")){	
					
					Base.logger.error("No existen datos [RetCode]:" + r.getRetCode() + " [msg]:" + r.getRetDescError());
                    JOptionPane.showMessageDialog(null, r.getRetDescError() != null 
                    		&& !r.getRetDescError().equals("") 
                    		&& !r.getRetDescError().equals("0") ? r.getRetDescError() : "No se encuentra la cuenta", 
            ...
            case 2:
                ...
                return ICajaView._WAITFORACTION;
    				...
    				JOptionPane.showMessageDialog(null, "Problemas al consultar Cuenta:"+ e.getStackTrace(), "Continuar", JOptionPane.INFORMATION_MESSAGE);
    				return 20;
    			}
