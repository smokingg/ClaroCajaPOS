 ...
 *
 */
public class TrxTarjetaTbkManualDirectoOffline extends MedioPago implements ITrxBase {
    int estado = 0;
    String[] opt;
    int index = 0;
    String tipoTarjeta = "";
    ...
    public int execute(ICajaView vista, int key, Datos data){ 
        ...
        switch( estado ) {
            case 0:
                ...
                opt = new String[2];
                opt[0] = "CREDITO TRANSBANK";
                opt[1] = "DEBITO TRANSBANK";
...
//                }
            ...
            case 8:
                ...
                vista.hideAllEntries();
                vista.setEntryTitle( "TARJETA CARGA MANUAL", true );
                vista.setEntryTextLabel("N° Cuotas: ", true);
                vista.setEntryMessage( "Ingrese N° de cuotas", true );
                vista.setEntryText("", true, false, false,"[0-9]{1,9}","Valor inválido");
                return ICajaView._WAITFORACTION;
            case 9:
                estado = 4;
                
                if( vista.getEntryText().equals("") || vista.getEntryText().length() > 9 ){
                    estado = 9;
                    vista.hideAllEntries();
                    vista.setEntryTitle( "TARJETA CARGA MANUAL", true );
                    vista.setEntryTextLabel("N° Cuotas: ", true);
                    vista.setEntryMessage( "N° no puede ser vacío", true );
                    vista.setEntryText("", true, false, false,null,null);
                    return ICajaView._WAITFORACTION;
                }
                String numCuotas = vista.getEntryText();
                this.datos.setValue("NumeroCuotas", numCuotas );
                return ICajaView._NOWAITFORACTION;
